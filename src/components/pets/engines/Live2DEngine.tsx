...
interface Live2DMotionOptions {
  group: string;
  index: number;
}

function motionFor(manifest: PetManifest, action: PetAction): Live2DMotionOptions {
  return manifest.actions[action].options as unknown as Live2DMotionOptions;
}
...
async function playMotion(model: Live2DModel, action: PetAction, manifest: PetManifest) {
  if (action === "sleep") {
    return;
  }
  const mapping = motionFor(manifest, action);
  try {
    await model.motion(mapping.group, mapping.index, MotionPriority.FORCE);
  } catch (err) {
    console.error("[Live2DEngine] motion() failed", { action, err });
  }
}

export function Live2DEngine({ size, manifest }: PetEngineProps) {
  ...
  useEffect(() => {
    ...
    const modelUrl = petAssetUrl(manifest, manifest.actions.idle.src);
    ...
    (async () => {
      try {
        ...
        await playMotion(loaded, usePetStore.getState().currentAction, manifest);
        ...
        unsubscribeStore = usePetStore.subscribe((state) => {
          ...
          if (next === "sleep") {
            ...
            internalModelRef.eyeBlink = origEyeBlink;
            void playMotion(model, next, manifest);
          }
        });
        ...
        console.error("[Live2DEngine] failed to load Live2D model", err);
